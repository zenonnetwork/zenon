...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "Zenon" is a composite category enabling all Zenon-related debug output
            if (ptrCategory->count(std::string("Zenon"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Zenon
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Zenon
// Mac: ~/Library/Application Support/Zenon
// Unix: ~/.Zenon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zenon";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Zenon.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty Zenon.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override Zenon.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Zenond.pid"));
