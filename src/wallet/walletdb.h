...
public:
    ...
    bool WriteZZNNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZZNNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZZNNSeed_deprecated(uint256& seed);
    bool EraseZZNNSeed();
    bool EraseZZNNSeed_deprecated();

    bool WriteZZNNCount(const uint32_t& nCount);
    bool ReadZZNNCount(uint32_t& nCount);
