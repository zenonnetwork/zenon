...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zznnspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ZNN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zZNN send
enum ZerocoinSpendStatus {
    ZZNN_SPEND_OKAY = 0,                            // No error
    ZZNN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZZNN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZNN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZNN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZNN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZNN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZNN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZNN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZNN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZNN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZZNN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZZNN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZZNN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZZNN_SPENT_USED_ZZNN = 14,                      // Coin has already been spend
    ZZNN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZZNN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZZnnBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZZNNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzznnAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzZNNTracker> zznnTracker;
    ...
    void setZWallet(CzZNNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zznnTracker = std::unique_ptr<CzZNNTracker>(new CzZNNTracker(strWalletFile));
    }

    CzZNNWallet* getZWallet() { return zwalletMain; }
    ...
    void setZZnnAutoBackups(bool fEnabled)
    {
    ...
    /** zZNN reset */
    boost::signals2::signal<void()> NotifyzZNNReset;
