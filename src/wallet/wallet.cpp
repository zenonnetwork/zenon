...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zznnTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zznnTracker->ListMints(true, true, true);
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if ((GetBoolArg("-zznnstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = zznnTracker->ListMints(true, true, fUpdate);
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to znn address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
std::string CWallet::GetUniqueWalletBackupName(bool fzznnAuto) const
{
...
void CWallet::ZZnnBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozznnbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zznnbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zznnbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no zZNN.
    /*
