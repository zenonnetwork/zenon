...
CzZNNWallet::CzZNNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zznn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZNNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZNNSeed_deprecated()) {
                    LogPrintf("%s: Updated zZNN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zznn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zznn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzZNNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZNNCount(nCountLastUsed);
    else if (!walletdb.ReadZZNNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZNNWallet::Lock()
{
    seedMaster = 0;
}

void CzZNNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZNNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZNN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZNNWallet::LoadMintPoolFromDB()
{
...
void CzZNNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZNNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZNNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zznnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzZNNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZNN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zznnTracker which also adds to database
    pwalletMain->zznnTracker->Add(dMint, true);
...
void CzZNNWallet::SeedToZZNN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZNNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZNNWallet::GenerateDeterministicZZNN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZNNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZNNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzZNNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
