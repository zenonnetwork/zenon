...
CzZNNTracker::CzZNNTracker(std::string strWalletFile)
{
...
CzZNNTracker::~CzZNNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzZNNTracker::Init()
{
...
bool CzZNNTracker::Archive(CMintMeta& meta)
{
...
bool CzZNNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzZNNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzZNNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzZNNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzZNNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzZNNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzZNNTracker::GetSerialHashes()
{
...
CAmount CzZNNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzZNNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzZNNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzZNNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzZNNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzZNNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzZNNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzZNNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzZNNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzZNNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzZNNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzZNNWallet* zZNNWallet)
{
    bool iszZNNWalletInitialized = (NULL != zZNNWallet);
    CMintMeta meta;
    ...
    if (! iszZNNWalletInitialized)
        zZNNWallet = new CzZNNWallet(strWalletFile);
    meta.isSeedCorrect = zZNNWallet->CheckSeed(dMint);
    if (! iszZNNWalletInitialized)
        delete zZNNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzZNNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzZNNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzZNNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzZNNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzZNNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzZNNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzZNNWallet* zZNNWallet = new CzZNNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zZNNWallet);
        }
        delete zZNNWallet;
        LogPrint("zero", "%s: added %d dzznn from DB\n", __func__, listDeterministicDB.size());
    }
