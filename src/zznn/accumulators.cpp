...
bool InitializeAccumulators(const int nHeight, int& nHeightCheckpoint, AccumulatorMap& mapAccumulators)
{
    ...
    if (nHeight == Params().Zerocoin_Block_RecalculateAccumulators() && Params().NetworkID() != CBaseChainParams::MAIN) {
    ...
    if (nHeight >= Params().Zerocoin_Block_V2_Start()) {
        ...
        if (nHeight <= Params().Zerocoin_Block_V2_Start() + 20 && Params().NetworkID() != CBaseChainParams::MAIN) {
...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zZNN need to be disabled at the same time!!
...
void AccumulateRange(CoinWitnessData* coinWitness, int nHeightEnd)
{
    ...
    while (pindex && pindex->nHeight <= nHeightEnd) {
        ...
        // 10 blocks were accumulated twice when zZNN v2 was activated
        ...
        CBlockIndex* pindexCheckpoint)
{
    try {
        ...
        if (GetAccumulatorValue(nHeightCheckpoint, coin.getDenomination(), bnAccValue)) {
            if(!bnAccValue && Params().NetworkID() == CBaseChainParams::MAIN){
