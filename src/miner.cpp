...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zznns that have been in mempool long
                    //and higher priority to zznns that are large in value
                    ...
                    // zZNN spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zZNN spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zZnn serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
//                //For the period before v2 activation, zZNN will be disabled and previous block's checkpoint is all that will be needed
...
//                    LogPrintf("%s: failed to find zZNN with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
//                    return NULL;
//                }
//
//                //Sign block with the zZNN key
//                if (!SignBlockWithKey(*pblock, key)) {
//                    LogPrintf("BitcoinMiner(): Signing new block with zZNN key failed \n");
//                    return NULL;
//                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZenonMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zznnTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zznnTracker->ListMints(true, true, true); //update the state
        }
        return error("ZNNMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ZenonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Zenon-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ZenonMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
