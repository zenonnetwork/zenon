...
#include "ui_zznncontroldialog.h"
...
bool CZZnnControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZZnnControlDialog::COLUMN_DENOMINATION || column == ZZnnControlDialog::COLUMN_VERSION || column == ZZnnControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZZnnControlDialog::ZZnnControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZZnnControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZZnnControlDialog::~ZZnnControlDialog()
{
    delete ui;
}

void ZZnnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZnnControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZZnnControlWidgetItem *itemMint = new CZZnnControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zznnTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zznnTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zZNN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zZNN seed used to mint this zZNN is not the same as currently hold in the wallet";
            else
...
void ZZnnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZnnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZZnn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZnnControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZZnnControlDialog::GetSelectedMints()
{
...
void ZZnnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
