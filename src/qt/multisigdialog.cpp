...
void MultisigDialog::on_importAddressButton_clicked(){
    ...
    if(sRedeem.empty()){
        ui->addMultisigStatus->setStyleSheet("QLabel { color: #F41954; }");
...
void MultisigDialog::on_createButton_clicked()
{
    ...
    try{
        ...
        if (CoinControlDialog::coinControl->HasSelected()) {
            ...
            for(int i = 0; i < ui->inputsList->count(); i++){
                ...
                if(txIdLine->text().isEmpty()){
                    ui->createButtonStatus->setStyleSheet("QLabel { color: #F41954; }");
        ...
        ui->createButtonStatus->setStyleSheet("QTextEdit{ color: #F41954; }");
...
bool MultisigDialog::createMultisigTransaction(std::vector<CTxIn> vUserIn, std::vector<CTxOut> vUserOut, std::string& feeStringRet, std::string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw std::runtime_error("Not enough ZNN provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
            ...
            throw std::runtime_error("Not enough ZNN provided to cover fee");
        }
...
void MultisigDialog::on_signButton_clicked()
{
    ...
    try{
        ...
        ui->signButtonStatus->setStyleSheet("QTextEdit{ color: #F41954; }");
