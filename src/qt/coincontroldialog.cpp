...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::ZNN;
    ...
    // turn labels "red"
    l5->setStyleSheet((nBytes >= MAX_FREE_TRANSACTION_CREATE_SIZE) ? "color: #F41954;" : ""); // Bytes >= 1000
    l6->setStyleSheet((dPriority > 0 && !fAllowFree) ? "color: #F41954;" : "");               // Priority < "medium"
    l7->setStyleSheet((fDust) ? "color: #F41954;" : "");                                      // Dust = "yes"
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Zenon address. In tree mode, address is not shown again for direct wallet address outputs
