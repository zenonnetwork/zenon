...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Zenon address (e.g. %1)").arg("ZQoL2x72i4FLujpsAm1cqqXUU77RgD1cpV"));
...
void setupAliasWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Node Alias (e.g. %1)").arg("node1"));
#endif
}
...
void setupIPWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(bitcoinAddressFont());
#if QT_VERSION >= 0x040700
    widget->setPlaceholderText(QObject::tr("Enter the VPS IP (e.g. %1)").arg("127.0.0.1:35993"));
#endif
}
...
void setupPrivKeyWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(bitcoinAddressFont());
#if QT_VERSION >= 0x040700
    widget->setPlaceholderText(QObject::tr("Enter a Node PivKey (e.g. %1)").arg("7oPq5jVXnM74gh879aaRgCi8gQmyYbJUifTRYhsDqhgrk5nbaUf"));
#endif
}
...
{
    // return if URI is not valid or is no Zenon: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::ZNN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert Zenon:// to Zenon:
    //
    //    Cannot handle this later, because Zenon:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ZNN, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open Zenon.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zenon.lnk";
}
...
{
    return GetAutostartDir() / "Zenon.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Zenon.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Zenon\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Zenon app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Zenon app to startup item list
...
QString loadStyleSheet(QString const& stylesheetName)
{
    QString styleSheet;
    QSettings settings;
    QString cssName;
