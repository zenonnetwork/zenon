...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::ZNN, nValue);
...
static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(), const std::string& Prepend = std::string(), int64_t* pSum = NULL)
{
    ...
    if (!Highlight.empty()) {
        List[n++] = std::string("<font color=\"") + ((Delta > 0) ? "#6FF34D" : "#F41954") + "\">" + ValueToString(Delta, true) + "</font>";
        *pSum += Delta;
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (Zenon.conf).");
            QMessageBox::warning(this, "Zenon Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#6FF34D; bgcolor:#181718;}\n a, span { font-family: moZenonpace; }\n span.addr {color:#6FF34D; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #181718;}\n td.d0 {font-weight: bold; color:#6FF34D;}\n h2, h3 { white-space:nowrap; color:#6FF34D;}\n a { color:#6FF34D; text-decoration:none; }\n a.nav {color:#6FF34D;}\n";
