...
// Copyright (c) 2018 The Zenon Ltd developers
...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZNN);
    ...
    if (!settings.contains("nAnonymizeZenonAmount"))
        settings.setValue("nAnonymizeZenonAmount", 1000);

    nAnonymizeZenonAmount = settings.value("nAnonymizeZenonAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZenonAmount"))
        SoftSetArg("-anonymizezenonamount", settings.value("nAnonymizeZenonAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Zenon.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZenonAmount:
            return QVariant(nAnonymizeZenonAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeZenonAmount:
            nAnonymizeZenonAmount = value.toInt();
            settings.setValue("nAnonymizeZenonAmount", nAnonymizeZenonAmount);
            emit anonymizeZenonAmountChanged(nAnonymizeZenonAmount);
            break;
        case CoinControlFeatures:
