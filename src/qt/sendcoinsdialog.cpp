...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_ZNN, true));
        if (!ctx.isValid()) {
...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    if (txFee > 0) {
        ...
        questionString.append("<hr /><span style='color:#F41954;'>");
...
{
    ui->sliderSmartFee->setVisible(!ui->checkSwiftTX->isChecked());
    ui->labelSmartFee->setVisible(!ui->checkSwiftTX->isChecked());
//    ui->labelSmartFee2->setVisible(!ui->checkSwiftTX->isChecked());
    ui->labelSmartFee3->setVisible(!ui->checkSwiftTX->isChecked());
//    ui->labelFeeEstimation->setVisible(!ui->checkSwiftTX->isChecked());
    ui->labelSmartFeeNormal->setVisible(!ui->checkSwiftTX->isChecked());
    ui->labelSmartFeeFast->setVisible(!ui->checkSwiftTX->isChecked());
    ...
    ui->radioCustomAtLeast->setVisible(ui->radioCustomFee->isChecked() && !ui->checkBoxMinimumFee->isChecked());
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        CoinControlDialog::coinControl->destChange = CNoDestination();
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color: #F41954;}");
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Zenon address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
