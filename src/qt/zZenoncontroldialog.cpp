...
#include "ui_zZenoncontroldialog.h"
...
std::list<std::string> ZZenonControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZenonControlDialog::listMints;

ZZenonControlDialog::ZZenonControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZenonControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZenonControlDialog::~ZZenonControlDialog()
{
    delete ui;
}

void ZZenonControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZenonControlDialog::updateList()
{
...
void ZZenonControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZenonControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZenon_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZenonControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZenonControlDialog::GetSelectedMints()
{
...
void ZZenonControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
